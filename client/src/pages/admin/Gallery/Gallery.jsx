import { useEffect, useState } from "react";
import { Edit, Images, Plus, Trash2 } from "lucide-react";

import {
  getGallery,
  createGallery,
  updateGallery,
  deleteGallery,
} from "../../../api/galleryApi";

import GalleryForm from "./GalleryForm";
import { useAuth } from "../../../context/AuthContext";

function Gallery() {
  const [gallery, setGallery] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const { token } = useAuth();

  const [formOpen, setFormOpen] = useState(false);
  const [formLoading, setFormLoading] = useState(false);
  const [editingGallery, setEditingGallery] = useState(null);
  const [deletingId, setDeletingId] = useState(null);

  useEffect(() => {
    const fetchGallery = async () => {
      try {
        setError("");

        const data = await getGallery();

        setGallery(data);
      } catch (error) {
        setError(error.response?.data?.message || "Unable to load gallery.");
      } finally {
        setLoading(false);
      }
    };

    fetchGallery();
  }, []);

  const handleCreateGallery = async (galleryData) => {
    setFormLoading(true);

    try {
      const newGalleryItem = await createGallery(galleryData, token);

      setGallery((previous) => [newGalleryItem, ...previous]);

      setFormOpen(false);
    } catch (error) {
      throw error;
    } finally {
      setFormLoading(false);
    }
  };

  const handleUpdateGallery = async (galleryData) => {
    setFormLoading(true);

    try {
      const updatedGalleryItem = await updateGallery(
        editingGallery._id,
        galleryData,
        token,
      );

      setGallery((previous) =>
        previous.map((item) =>
          item._id === updatedGalleryItem._id ? updatedGalleryItem : item,
        ),
      );

      setFormOpen(false);
      setEditingGallery(null);
    } catch (error) {
      throw error;
    } finally {
      setFormLoading(false);
    }
  };

  const handleDeleteGallery = async (id) => {
    const confirmed = window.confirm(
      "Are you sure you want to delete this gallery image?",
    );

    if (!confirmed) {
      return;
    }

    setDeletingId(id);

    try {
      await deleteGallery(id, token);

      setGallery((previous) => previous.filter((item) => item._id !== id));
    } catch (error) {
      setError(
        error.response?.data?.message || "Unable to delete gallery image.",
      );
    } finally {
      setDeletingId(null);
    }
  };

  return (
    <div>
      {/* Page Header */}
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">
            Gallery
          </h1>

          <p className="mt-1 text-sm text-slate-500">
            Manage the images displayed on your website.
          </p>
        </div>

        <button
          type="button"
          onClick={() => {
            setEditingGallery(null);
            setFormOpen(true);
          }}
          className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800"
        >
          <Plus size={18} />
          Add Image
        </button>
      </div>

      {/* Error */}
      {error && (
        <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {error}
        </div>
      )}

      {/* Loading */}
      {loading && (
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          <p className="text-sm text-slate-500">Loading gallery...</p>
        </div>
      )}

      {/* Empty State */}
      {!loading && !error && gallery.length === 0 && (
        <div className="rounded-xl border border-slate-200 bg-white p-10 text-center shadow-sm">
          <Images size={40} className="mx-auto text-slate-300" />

          <h2 className="mt-4 text-lg font-semibold text-slate-900">
            No gallery images yet
          </h2>

          <p className="mt-1 text-sm text-slate-500">
            Add your first image to display it on the website.
          </p>
        </div>
      )}

      {/* Gallery Table */}
      {!loading && gallery.length > 0 && (
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px] text-left">
              <thead className="border-b border-slate-200 bg-slate-50">
                <tr>
                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Image
                  </th>

                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Title
                  </th>

                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Category
                  </th>

                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Status
                  </th>

                  <th className="px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Actions
                  </th>
                </tr>
              </thead>

              <tbody className="divide-y divide-slate-100">
                {gallery.map((item) => (
                  <tr key={item._id} className="transition hover:bg-slate-50">
                    {/* Image */}
                    <td className="px-6 py-4">
                      <div className="h-14 w-20 overflow-hidden rounded-lg bg-slate-100">
                        {item.image ? (
                          <img
                            src={item.image}
                            alt={item.title}
                            className="h-full w-full object-cover"
                          />
                        ) : (
                          <div className="flex h-full w-full items-center justify-center text-slate-400">
                            <Images size={20} />
                          </div>
                        )}
                      </div>
                    </td>

                    {/* Title */}
                    <td className="px-6 py-4">
                      <p className="font-semibold text-slate-900">
                        {item.title}
                      </p>

                      {item.description && (
                        <p className="mt-1 max-w-md truncate text-sm text-slate-500">
                          {item.description}
                        </p>
                      )}
                    </td>

                    {/* Category */}
                    <td className="px-6 py-4">
                      <span className="text-sm text-slate-600">
                        {item.category || "General"}
                      </span>
                    </td>

                    {/* Status */}
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                          item.status === "active"
                            ? "bg-green-100 text-green-700"
                            : "bg-slate-100 text-slate-600"
                        }`}
                      >
                        {item.status}
                      </span>
                    </td>

                    {/* Actions */}
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            setEditingGallery(item);
                            setFormOpen(true);
                          }}
                          className="cursor-pointer rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
                          aria-label={`Edit ${item.title}`}
                        >
                          <Edit size={17} />
                        </button>

                        <button
                          type="button"
                          onClick={() => handleDeleteGallery(item._id)}
                          disabled={deletingId === item._id}
                          className="cursor-pointer rounded-lg p-2 text-slate-500 transition hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-50"
                          aria-label={`Delete ${item.title}`}
                        >
                          <Trash2 size={17} />
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      )}

      <GalleryForm
        isOpen={formOpen}
        onClose={() => {
          setFormOpen(false);
          setEditingGallery(null);
        }}
        onSubmit={editingGallery ? handleUpdateGallery : handleCreateGallery}
        initialData={editingGallery}
        loading={formLoading}
      />
    </div>
  );
}

export default Gallery;
